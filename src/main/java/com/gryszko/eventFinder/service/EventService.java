...
public class EventService {
    ...
    public List<EventResponse> getAllEventsWithStartDateTodayOrLater(String city, String keyWord) {
        ...
        if (!Strings.isNullOrEmpty(city) && Strings.isNullOrEmpty(keyWord)) {
            ...
            return eventRepository.findAllByDescriptionIgnoreCaseContainingOrTitleIgnoreCaseContainingAndStartingDateGreaterThanEqualOrderByStartingDateAsc(keyWord, keyWord,today)
                    .stream()
                    .map(eventMapper::mapEventEntityToEventResponse)
                    .collect(Collectors.toList());
        } else if (!Strings.isNullOrEmpty(city) && !Strings.isNullOrEmpty(keyWord)) {
            return eventRepository.findAllByCityLikeAndDescriptionIgnoreCaseContainingOrTitleIgnoreCaseContainingAndStartingDateGreaterThanEqualOrderByStartingDateAsc(city, keyWord, keyWord,today)
                    .stream()
                    .map(eventMapper::mapEventEntityToEventResponse)
                    .collect(Collectors.toList());
        }
